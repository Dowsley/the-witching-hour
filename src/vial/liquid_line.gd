...
func _remove_invalid_points() -> void:
	...
	for i in range(get_point_count()):
		...
		elif collided_object is Vial:
			collided_object.add_liquid(liquid_type, Globals.unit_per_liquid_line_point)
...
func _collides_with_world(global_point: Vector2) -> Object:
	...
	for r in result:
		...
		if is_valid_vial_collider(collider):
			break
	if collider == null or not collider is Vial:
		return null
	return collider if collider != origin_vial and is_valid_vial_collider(collider) else null


func is_valid_vial_collider(collider: Object) -> bool:
	if not collider is Vial:
		return false
